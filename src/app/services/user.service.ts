...
export class UserService {
  ...
  async decrementIngredientFromUser(userId: string, ingredientId: string) {
    const userDocRef = this.af.collection<User>('usuarios').doc(userId);
    try {
      await this.af.firestore.runTransaction(async (transaction) => {
        const userDoc = await transaction.get(userDocRef.ref);
        ...
        if (ingredientIndex !== -1) {
          ...
          if (parseInt(count) > 1) {
            ...
            existentes.splice(ingredientIndex, 1);
          }
        }
      ...
      console.error("Error decrementing ingredient from user:", error);
      return false;
    }
  }

  async addIngredientToConsumidos(userId: string, ingredientId: string) {
    const userDocRef = this.af.collection<User>('usuarios').doc(userId);
    try {
      await this.af.firestore.runTransaction(async (transaction) => {
        const userDoc = await transaction.get(userDocRef.ref);
        ...
        if (!consumidos.includes(ingredientId)) {
          consumidos.push(ingredientId);
        }

        transaction.update(userDocRef.ref, { consumidos: consumidos });
      });
      return true;
    } catch (error) {
      console.error("Error adding ingredient to consumidos:", error);
      return false;
    }
  }

  async removeIngredientFromConsumidos(userId: string, ingredientId: string) {
    const userDocRef = this.af.collection<User>('usuarios').doc(userId);
    try {
      await this.af.firestore.runTransaction(async (transaction) => {
        const userDoc = await transaction.get(userDocRef.ref);
        ...
        const index = consumidos.indexOf(ingredientId);
        if (index > -1) {
          consumidos.splice(index, 1);
        }
      ...
      console.error("Error removing ingredient from consumidos:", error);
      return false;
    }
  }
